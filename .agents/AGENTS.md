...
- **Mandatory Suite-Level Testing:** If you modify a `beforeAll`, `beforeEach`, or any setup block within a test file, you MUST run ALL test cases within that specific file to ensure your changes did not break the other scenarios. Never run just the single test you were working on if you touched global suite setup code.
- **Stop and Ask for Help:** If you are stuck at any place while implementing or debugging anything, try a maximum of 4 to 5 times. If you are still not successful, you MUST stop and ask the user for help. Do not enter an endless retry loop. Clearly explain to the user where you are stuck, what is not working, and why you are in a loop.
...
## UI & Modal Handling Best Practices
- **SweetAlert2 (Toasts & Modals):** Do not rely on `.filter({ state: 'visible' })` for SweetAlert popup buttons, as visibility state transitions can cause Playwright timeouts. Use robust native classes (e.g., `.swal2-confirm` for Yes/Submit and `.swal2-cancel` for Cancel) instead of generic `getByRole` filtering whenever interacting with these specific alerts.
